...
// Funky reduce() is more complicated, because of the sequential dependency among the different function applications.
Array.prototype.reduceAsync = function(f, acc, callback) {
    var that = this;

    // We can use a trick similar to the one from the 'serial.js' demo of last lecture.
    function oneStep(i, acc) {
        if (i >= that.length) {
            callback(acc);
            return;
        }

        f(that[i], acc, function(value) { oneStep(i+1, value) });
    }
...
function search(text, output) {
    ...
    nearbySearch(text, function(results) {
        results.reduceAsync(function(result, best, valueIs) {
            getDetails(result.place_id,
                       function(place) {
                           valueIs(Math.max(place.utc_offset, best))
                       })
        }, -Infinity, output)
    });
}
